<template>
  ...
  <div class="buscador-container"> 
    ...
    <div v-if="resultado">
      ...
      <!-- Inventario -->
      <div v-if="tipo === 'inventarios'" :key="resultado.id" class="resultado-card" @click="irAEdicion(resultado)">
        <h2>{{ resultado.ingrediente_id }}</h2>
        <p>{{ resultado.usuario_id }}</p>
...
function irAEdicion(item) {
  ...
  } else if (tipo.value === "inventarios") {
    // intenta pasar ambos por si tienes edición con ids
    query = {
      cantidad: item.cantidad,
      ingrediente: item.ingrediente_id ?? item.ingrediente?.id ?? item.ingrediente ?? null,
      usuario: item.usuario_id ?? item.usuario?.id ?? item.usuario ?? null,
    };
